...
fun babies_program (fileName, yearSt) =
  if null (tl(split_at(read_stdin(), #"\n"))) andalso null (hd(split_at(read_stdin(),  #"\n")))
  then print("There were no names entered to check so program is terminating")
  else
    let
      ...
      fun parseInfo(parseList : string list, nameToCheck : string list)=
        let
          ...
          fun getTotal(foundBabyNameList : string list, lastYear:
            string,totalNonZero: int)=
            if null(tl (tl foundBabyNameList))
            then 
              if hd foundBabyNameList = int_to_string(0)
